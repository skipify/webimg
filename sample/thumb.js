...
Webimg('./test.jpg').width(100).thumb();

//按照指定宽高自动缩小，比例不合适时自动裁剪
Webimg('./test.jpg').height(50).width(100).thumb();
//=》等同于
Webimg('./test.jpg').resize(100,50).thumb();

//指定输出的名字
//指定名字如果包含 . 则不会增加扩展名，如果不包含.则会自动添加源文件的扩展名
//不包含 / 则会生成到与原图相同的目录 包含 / 则会当做一个最终的路径输出
//Webimg('./test.jpg').resize(100,50).outFile('thumb.jpg').thumb();
//等同于 
//Webimg('./test.jpg').resize(100,50).thumb('thumb.jpg'); 
//等同于 
Webimg('./test.jpg').resize(100,50).thumb('thumb'); 
